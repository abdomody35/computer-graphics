...
void drawPolygon(GrayscaleImage &image, const std::vector<Point> &points, Byte outlineColor = 255, bool fill = false, std::optional<Byte> fillColor = std::nullopt)
{
    int n = points.size();
    ...
    if (fillColor != outlineColor || !fill)
    {
        for (int i = 0; i < n; i++)
        {
